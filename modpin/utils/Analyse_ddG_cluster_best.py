...
def parse_user_arguments(*args, **kwds):
    ...
    parser.add_argument('-n', '--number_of_models', dest = 'nmodels', action = 'store', default = 5, type=int,
                        help = 'Minimum number of models to calculate the correlation (default is 5)')
...
def main():
 ...
 for perc in xrange(int(percentil),100,1):
  for rnk in xrange(rank,0,-1):
   for pthr in xrange(n_pval):
       ...
       if len(ave_list)<=nmodels: continue
 ...
 try:
    for (pv,rnk,perc) in sorted(rank_cluster.iterkeys(),key=lambda x: rank_cluster.get(x)[0],reverse=True):
      irank=irank+1
      pearson,prob,m,c,points = rank_cluster.get((pv,rnk,perc))
      fo.write(" %10d\t%10.5f\t%10.1e\t%10d\t%10.5f\t%10d\t%10.1e\t%10.5f\t%10.5f\n"%(irank,float(perc),float(pv),int(rnk),float(pearson),int(points),float(prob),float(m),float(c)))
 ...
 for forms,ene in ddg_real_dict.iteritems():
         if ddg_pred_dict.has_key(forms):
            ...
            ave_list.append(factor*ddg_pred_dict.get(forms))
 if len(ave_list)<=0:
      if verbose: sys.stderr.write("Exit. With the starting conditions there are no solutions, try other options\n")
      exit(0)
