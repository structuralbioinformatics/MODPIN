...
# from SBI.structure import SSpair
...
def options(*args, **kwds):
    parser = argparse.ArgumentParser(prog = 'DS2SQL',  formatter_class = argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-d', '--database',     dest     = "database",
                        action = "store",       required = True,  help = "DS output dir.")
...
def DS2SQL(database, looplist, sqlfile, verbose):
    ...
    for dsfile in Path.list_files(database):
        ...
        if verbose: sys.stderr.write("Retrieving data for subclass {0} ...\n".format(subclasstype))
        ...
        for line in sql_in:
            ...
            if line.startswith('CONSENSUS & MULTIPLE ALIGNEMENT IN THE'):
                ...
                classification.subclasses = Subclass(tuple([data[0].strip(),data[3].strip()]), data[4])
            ...
            if read:
                ...
                elif line.startswith('--------- CONSENSUS THORNTON       :'):   workscls.add_consensus(dataline, 'DS', loops)
                elif line.startswith('--------- CONSENSUS TOPOLOGY'):           workscls.add_topology(dataline, 'DS')
...
if __name__ == "__main__":
    options = options()
    DS2SQL(options.database, options.looplist, options.sqlfile, options.verbose)
